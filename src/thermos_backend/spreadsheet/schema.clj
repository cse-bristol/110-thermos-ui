      ...
      [:map
       ...
       [:rows [:sequential
               [:multi {:dispatch (comp sheet/to-keyword :parameter) :error/message "unknown parameter"}
                ...
                [:diversity-rate [:map [:value number?]]]
                [:diversity-limit [:map [:value number?]]]
                ...
                [:objective-scale [:map [:value number?]]]
                [:objective-precision [:map [:value number?]]]
                [:edge-cost-precision [:map [:value number?]]]
